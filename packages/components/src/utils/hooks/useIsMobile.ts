...
export const useIsMobile = (defaultValue = false): { isMobile: boolean; parser: UAParser } => {
  ...
  useEffect(() => {
    setIsMobile(
      ["mobile", "tablet", "wearable", "smarttv"].includes(parser.getDevice().type || "") || isMobileByDetect || defaultValue
    );
  }, [defaultValue, parser, windowDimensions.width, windowDimensions.height]);
